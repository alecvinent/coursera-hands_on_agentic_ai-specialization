...
from mcp_portfolio.auth import AuthContext, Unauthenticated, authenticate, require
from mcp_portfolio.config import Settings
from mcp_portfolio.metrics import MetricsRegistry
from mcp_portfolio.models import ApiCredential, OPERATOR_ROLES
...
def render_dashboard(deps: OpsDeps, ctx: AuthContext) -> str:
    ...
    return (
        "<html><head><title>MCP Portfolio Dashboard</title></head><body>"
...
def create_ops_app(deps: OpsDeps):
    ...
    app = FastAPI(title="MCP Portfolio Ops")
...
def run_ops(deps: OpsDeps, host: str = "127.0.0.1", port: int = 8001) -> None:
    import uvicorn
    uvicorn.run(create_ops_app(deps), host=host, port=port)

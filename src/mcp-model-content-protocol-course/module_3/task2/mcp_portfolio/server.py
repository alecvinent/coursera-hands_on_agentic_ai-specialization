...
from mcp_portfolio.auth import authenticate
...
from mcp_portfolio.ops_app import OpsDeps
from mcp_portfolio.ratelimit import RateLimiter
from mcp_portfolio.resilience import CircuitBreaker
from mcp_portfolio.resources import ResourceService
from mcp_portfolio.store import MockStore
from mcp_portfolio.tools import TOOL_ROLES, ToolService
...
def build_server(
    ...
    ctx = authenticate(settings.api_keys, api_key)
    deps = deps or ServerDeps(settings)
    mcp = MCPServer("mcp-portfolio")
    ...
    async def customer_profile(customer_id: str) -> dict:
        return await deps.resources.read(f"retail://customers/{customer_id}", api_key)

    @mcp.resource(
        "retail://customers/{customer_id}/orders", mime_type="application/json"
    )
    async def customer_orders(customer_id: str) -> dict:
        return await deps.resources.read(
            f"retail://customers/{customer_id}/orders", api_key
        )

    @mcp.resource(
        "retail://customers/{customer_id}/tickets", mime_type="application/json"
    )
    async def customer_tickets(customer_id: str) -> dict:
        return await deps.resources.read(
            f"retail://customers/{customer_id}/tickets", api_key
        )

    @mcp.resource("retail://orders/{order_id}", mime_type="application/json")
    async def order_detail(order_id: str) -> dict:
        return await deps.resources.read(f"retail://orders/{order_id}", api_key)

    @mcp.resource("retail://orders/status/{status}", mime_type="application/json")
    async def orders_by_status(status: str) -> dict:
        return await deps.resources.read(f"retail://orders/status/{status}", api_key)

    @mcp.resource("retail://inventory/low-stock", mime_type="application/json")
    async def low_stock_inventory() -> dict:
        return await deps.resources.read("retail://inventory/low-stock", api_key)

    @mcp.resource("retail://inventory/{product_id}", mime_type="application/json")
    async def inventory_detail(product_id: str) -> dict:
        return await deps.resources.read(f"retail://inventory/{product_id}", api_key)

    @mcp.resource("retail://products", mime_type="application/json")
    async def product_catalog() -> dict:
        return await deps.resources.read("retail://products", api_key)

    @mcp.resource("retail://products/{product_id}", mime_type="application/json")
    async def product_detail(product_id: str) -> dict:
        return await deps.resources.read(f"retail://products/{product_id}", api_key)
    ...
    async def create_order(
        ...
        return await deps.tools.create_order(
            api_key,
    ...
    async def update_order_status(
        ...
        return await deps.tools.update_order_status(
            api_key,
            {
                "order_id": order_id,
    ...
    async def update_inventory(
        ...
        return await deps.tools.update_inventory(
            api_key,
    ...
    async def reserve_inventory(
        ...
        return await deps.tools.reserve_inventory(
            api_key,
            {"product_id": product_id, "quantity": quantity},
        )
    ...
    async def create_support_ticket(
        ...
        return await deps.tools.create_support_ticket(
            api_key,
            {
    ...
    async def get_task_status(task_id: str) -> dict:
        from mcp_portfolio.auth import Forbidden, Unauthenticated, require
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="MCP Enterprise Integration Portfolio server")
    ...
    parser.add_argument("--api-key", default=os.environ.get("MCP_PORTFOLIO_API_KEY", ""))
    ...
    if args.transport == "stdio":
        ...
        from mcp_portfolio.ops_app import create_ops_app
